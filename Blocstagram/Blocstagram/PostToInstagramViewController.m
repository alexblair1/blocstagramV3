...
//  Blocstagram
//
//  Created by Stephen Blair on 6/25/15.
...
//replace uicollectionviewcell with a subclass for assignment
- (void) viewDidLoad{
    [super viewDidLoad];
...
- (void) addCIImageToCollectionView:(CIImage *)CIImage withFilterTitle:(NSString *)filterTitle {
    UIImage *image = [UIImage imageWithCIImage:CIImage scale:self.sourceImage.scale orientation:self.sourceImage.imageOrientation];//conversion
    
    if (image) {
        // Decompress image - forces the uiimage to draw, and then saves the drawn uiimage
...
- (void) addFiltersToQueue {
    ...
    //Posterize - Assignment
    
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *gaussianBlur = [CIFilter filterWithName:@"CIColorPosterize"];
        
        if (gaussianBlur) {
            [gaussianBlur setValue:sourceCIImage forKey:kCIInputImageKey];
            [self addCIImageToCollectionView:gaussianBlur.outputImage withFilterTitle:NSLocalizedString(@"Posterize", @"posterize filter")];
        }
    }];
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        ...
        if (vignetteFilter) {
            ...
            CIImage *result = vignetteFilter.outputImage;
            ...
            [self addCIImageToCollectionView:result withFilterTitle:NSLocalizedString(@"Old", @"old compound filter")];
        }
